		...
		"ajax": {
            ...
            dataSrc : function (json){
	        	for(var i=0,len=json.data.length;i<len;i++){
	        		...
	        		for(var j=0,len1=json.group.length;j<len1;j++){
	        			if(json.group[j]['id'] == json.data[i]['jobGroup']) {
	        				json.data[i]['jobDesc'] = json.group[j]['title'];
						};
	        		};
				};
				return json.data;
	    ...
	    "columns": [
					{
						...
						"render": function ( data, type, row ) {
							...
							var temp = '';
							temp += I18n.jobinfo_field_jobgroup + '：' + row.jobDesc;
							temp += '<br>' + I18n.joblog_field_executorAddress + '：' + (row.executorAddress?row.executorAddress:'');
							temp += jobhandler;
